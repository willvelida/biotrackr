...
6. Runtime measurement health: read `.copilot-tracking/harness-evolution-metrics.md` and verify its header matches the 11-column schema (Date, Cycle, Sessions, Events, Pass, Fail, Error, Skip, Degraded, Timeout, TotalMs). Compare its most recent row against the raw store at `.copilot-tracking/observability/`: flag WARN when unpromoted records older than seven days exist, because promotion runs from the push gate and that gate can be bypassed. Note that `bash scripts/audit-harness.sh` already checks both mechanically — carry its result in rather than re-deriving it. The raw store is gitignored and absent by default, which is not a finding.
