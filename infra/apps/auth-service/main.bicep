...
resource acrPurgeImageTask 'Microsoft.ContainerRegistry/registries/tasks@2019-06-01-preview' = {
  name: 'purgeOldImage-${name}'
  parent: acr
  location: location
  properties: {
    status: 'Enabled'
    ...
    step: {
      ...
      arguments: [
        { 
          name: 'acr purge --filter "${name}:.*" --ago 0d --keep 2'
          value: ''
          isSecret: false
        }
      ]
