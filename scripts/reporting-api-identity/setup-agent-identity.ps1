<#
.SYNOPSIS
    Creates the Agent Identity Blueprint in Entra ID for Biotrackr Reporting Agent.
...
.DESCRIPTION
    ...
    This blueprint enables secure inter-service A2A authentication: Chat.Api acquires
    an agent identity token to call Reporting.Api endpoints, which validate the bearer
    token at the application level (defense-in-depth, ASI03/ASI07).
...
.EXAMPLE
    .\setup-agent-identity.ps1 -TenantId "00000000-0000-0000-0000-000000000000" -AgentBlueprintPrincipalName "Biotrackr Reporting Agent"
#>
...
$scope = @{
    adminConsentDescription = "Allow the Chat Agent to call the Reporting Agent on behalf of itself."
    adminConsentDisplayName = "Access Reporting Agent"
    id                      = $scopeId
    isEnabled               = $true
    type                    = "User"
    value                   = "access_agent"
...
Write-Host ""
Write-Host "Bicep parameters to set:"
Write-Host "  agentBlueprintClientId     $applicationId"
Write-Host ""
Write-Host "Chat.Api App Config to set:"
Write-Host "  Biotrackr:ReportingApiScope  api://$applicationId/.default"
Write-Host ""
